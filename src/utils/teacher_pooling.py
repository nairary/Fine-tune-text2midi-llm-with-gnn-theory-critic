...
class MultiTypeMeanPooling(nn.Module):
    def __init__(self, hidden_dim: int, node_types: Iterable[str], output_dim: int | None = None, pooling_mode: str = "mean"):
        ...
        if pooling_mode not in {"mean", "mean_max"}:
            raise ValueError(f"Unsupported pooling_mode='{pooling_mode}'. Expected one of ['mean', 'mean_max'].")
        self.hidden_dim = hidden_dim
        self.node_types = tuple(node_types)
        self.pooling_mode = pooling_mode
        self.per_type_dim = hidden_dim if self.pooling_mode == "mean" else hidden_dim * 2
    ...
    def forward(
        ...
        for node_type in self.node_types:
            ...
            if embeddings.size(0) == 0:
                pooled_mean = reference_tensor.new_zeros((num_graphs, self.hidden_dim))
                pooled_max = reference_tensor.new_zeros((num_graphs, self.hidden_dim))
            else:
